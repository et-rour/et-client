  ...
  <article class="rounded-xl border-2 border-gray-100 bg-white">
    <div class="flex items-start p-6">
      <div class="ml-4">
        ...
        <button
          class="px-2 py-1 border border-gray-400 rounded-md mt-5"
          @click="getAllFiles"
        >
          {{ $t("admin.reservations.myFiles") }}
        </button>
        <Spiner v-if="isLoadingFiles" />
        <div class="gap-3 my-4 grid grid-cols-1 lg:grid-cols-3">
          <div
            class="flex p-4 border border-gray-200 shadow-sm"
          ...
          >
            <div class="w-20 h-14 flex-shrink-0 border border-gray-300">
              ...
              >
                <div
                  class="w-full h-full flex justify-center items-center text-4xl"
                >
            ...
            <div class="ml-3 flex-1 w-24">
              ...
              <button
                class="my-btn mt-2 py-0 px-2 text-sm rounded-sm w-auto bg-red-500"
                @click="onClickDeltefile(file)"
              >
                {{ $t("general.delete") }}
              </button>
            </div>
...
export default {
  ...
  methods: {
    async getAllFiles() {
      const listRef = ref(
        storage,
        `Reservations/Reservation_${this.reservation.id}`
      );
      this.files = [];
      const filesUrls = [];
